            ...
            {loading ? (
              ...
              data.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">{item.course_name}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">{item.preferred_date || "-"}</td>
        ...
        {selected && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><span className="text-xs text-gray-500">교육명</span><p className="text-sm font-medium">{selected.course_name}</p></div>
              <div><span className="text-xs text-gray-500">희망 일자</span><p className="text-sm">{selected.preferred_date || "-"}</p></div>
            ...
            <div className="flex items-center gap-3 pt-2 border-t border-gray-200">
              ...
              {editable && (
                <select value={selected.status}
                  onChange={(e) => changeStatus(selected.id, e.target.value as EducationStatus)}
