...
const SECTIONS: SectionDef<unknown>[] = [
  ...
  { key: "company_stats", label: "통계 숫자", default: [{ num: "16", label: "년 업력" }, { num: "200+", label: "고객사" }, { num: "4", label: "전문 사업부" }, { num: "50+", label: "파트너사" }] },
  ...
  { key: "company_depts", label: "부서 정보", default: [{ name: "솔루션사업부", desc: "DATA / SW / SI 사업팀" }, { name: "영업부", desc: "공공영업 / 기업영업 / 교육영업" }, { name: "서비스사업부", desc: "기술지원 팀" }, { name: "사업지원부", desc: "리뉴얼 / 마케팅 / 영업지원" }] },
...
function ObjectFields({
  data, defaultData, sectionKey, onChange,
}: {
          ...
          {IMAGE_FIELDS.has(k) ? (
            <>
              ...
              {defaultData?.[k] && v !== defaultData[k] && (
                <button
                  onClick={() => onChange(k, defaultData[k])}
...
function CompanyAboutEditor() {
  ...
  const sendToPreview = useCallback((section: string, newData: unknown) => {
    iframeRef.current?.contentWindow?.postMessage(
      { type: "content-update", section, data: newData }, "*"
    );
  }, []);
  ...
  // iframe에서 field-click 수신 → 폼 스크롤
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === "field-click" && e.data.id) {
        setActiveField(e.data.id);
        // 폼에서 해당 필드로 스크롤
        ...
        if (fieldEl) {
          ...
          setTimeout(() => {
            (fieldEl as HTMLElement).style.outline = "";
            (fieldEl as HTMLElement).style.outlineOffset = "";
          }, 2000);
          // input/textarea에 포커스
        ...
        <div className="p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">실시간 미리보기 · 페이지 클릭→여기 스크롤</p>
            </div>
            ...
            {SECTIONS.map((section) => {
              ...
              return (
                <div key={section.key} id={`section-${section.key}`}
                  ...
                  {isArray ? (
                    <ArrayFields data={value as Record<string, string>[]} defaultData={section.default as Record<string, string>[]} sectionKey={section.key}
                      onChange={(idx, k, v) => updateArray(section.key, idx, k, v)} />
                  ) : (
                    <ObjectFields data={value as Record<string, string>} defaultData={section.default as Record<string, string>} sectionKey={section.key}
