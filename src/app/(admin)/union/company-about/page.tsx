...
function GenericPageEditor() {
  ...
  // 페이지 선택
  const [pageUrl, setPageUrl] = useState(PRESET_PAGES[0].path);
  const [urlInput, setUrlInput] = useState(PRESET_PAGES[0].path);
  ...
  const loadPage = useCallback((path: string) => {
    ...
    if (iframeRef.current) {
      iframeRef.current.src = `${PREVIEW_URL}${path}?_edit=1&t=${Date.now()}`;
    }
  }, []);
  ...
  const loadSectionData = useCallback(async (keys: string[]) => {
    try {
      const contents = await apiFetch<ContentResponse[]>("/api/admin/union/contents");
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === "editable-manifest") {
        ...
        console.log("[recv]", newPath, fields.length, "fields");
  ...
  const renderField = useCallback((field: ManifestField) => {
    ...
    return (
      <div key={field.id} data-field-id={field.id} className="mb-3">
        ...
        {field.type === "image" ? (
          <ImageUploadField value={value} onChange={url => updateField(field.id, url)} site="union" />
        ) : isTextarea ? (
    ...
    if (isArray) {
      ...
      for (const f of section.fields) {
        ...
        if (match) {
          ...
          // non-indexed fields (shouldn't happen in well-formed manifests)
      ...
      <div className="flex-1 border-r border-gray-200 flex flex-col bg-gray-100">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-white border-b border-gray-200 shrink-0">
          ...
          <div className="flex-1 flex items-center gap-2 ml-2">
            <span className="text-xs text-gray-400 font-mono shrink-0">{PREVIEW_URL}</span>
            <input
              value={urlInput}
              onChange={e => setUrlInput(e.target.value)}
              onKeyDown={e => { if (e.key === "Enter") loadPage(urlInput); }}
              className="flex-1 text-xs font-mono border border-gray-200 rounded px-2 py-1 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500"
              placeholder="/company"
            />
            <button onClick={() => loadPage(urlInput)}
              className="text-xs text-white bg-emerald-600 hover:bg-emerald-700 px-3 py-1 rounded font-medium shrink-0">열기</button>
          </div>
        ...
        <div className="flex items-center gap-1 px-4 py-1.5 bg-gray-50 border-b border-gray-200 shrink-0">
          {PRESET_PAGES.map(p => (
            <button key={p.path} onClick={() => loadPage(p.path)}
              className={`text-xs px-3 py-1 rounded transition-colors ${pageUrl === p.path ? "bg-emerald-600 text-white" : "text-gray-600 hover:bg-gray-200"}`}>
              {p.label}
            </button>
          ))}
        </div>
        <iframe ref={iframeRef} src={`${PREVIEW_URL}${pageUrl}?_edit=1`}
        ...
        <div className="p-5">
          ...
          {!loaded && (
            <div className="text-center py-16">
              <div className="w-8 h-8 border-2 border-gray-300 border-t-emerald-600 rounded-full animate-spin mx-auto mb-4" />
                  ...
                  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => saveSection(section.key)} disabled={saving !== null || !dirty.has(section.key)}
                        className="px-2.5 py-1 text-xs bg-emerald-50 text-emerald-700 rounded hover:bg-emerald-100 disabled:opacity-40 font-medium">
