...
public class AdminMenuService {
    ...
    public List<MenuResponse> getPublicMenuTree(String site) {
        ...
        List<MenuResponse> allVisible;
        if ("union".equals(normalized)) {
            allVisible = unionMenuRepo.findByIsExposedTrueOrderBySortOrderAsc().stream()
                    .map(this::toUnionResponse).collect(Collectors.toList());
        } else {
            allVisible = datawareMenuRepo.findByIsExposedTrueOrderBySortOrderAsc().stream()
                    .map(this::toDatawareResponse).collect(Collectors.toList());
        }

        // 숨긴 부모의 자식(및 손자 이하)도 재귀적으로 제외
        int prevSize;
        do {
            prevSize = allVisible.size();
            Set<Long> visibleIds = allVisible.stream()
                    .map(MenuResponse::getId).collect(Collectors.toSet());
            allVisible = allVisible.stream()
                    .filter(m -> m.getParentId() == null || visibleIds.contains(m.getParentId()))
                    .collect(Collectors.toList());
        } while (allVisible.size() < prevSize);

        return buildTree(allVisible);
    }
